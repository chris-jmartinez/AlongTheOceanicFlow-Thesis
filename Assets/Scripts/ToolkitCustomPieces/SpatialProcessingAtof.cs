...
namespace HoloToolkit.Unity.SpatialMapping.Tests
{
    ...
    /// </summary>
    public class SpatialProcessingAtof : Singleton<SpatialProcessingAtof>
    {
        ...
        [Tooltip("Minimum number of walls planes required in order to exit scanning/processing mode.")]
        ...
        private void SurfaceMeshesToPlanes_MakePlanesComplete(object source, System.EventArgs args)
        {
            ...
            if (floors.Count >= minimumFloors && walls.Count >= minimumWalls)
            {
                ...
                //In our application we want the ceiling to be invisible, so we deactivate its mesh renderer (will be invisible both on hololens and unity)
                if (ceilings.Count > 0)
                    foreach (GameObject ceiling in ceilings)
                        ceiling.GetComponent<MeshRenderer>().enabled = false;

                //Disable collider of the mesh not converted in planes
                SpatialMappingManager.Instance.transform.GetChild(0).gameObject.layer = 10; //Layer 10 is ignore gaze. For some reason maybe Hololens still detects gaze eve if the mesh of the room is deactivated. We try assigning also a ignore gaze layer
                SpatialMappingManager.Instance.transform.GetChild(0).transform.GetComponent<Collider>().enabled = false;
                SpatialMappingManager.Instance.DrawVisualMeshes = false;

                // Tell to the UI menu that the scan is completed, so we can display the message "Completed Scan"
                uiMenu.GetComponent<UiMenuHandler>().ScanningComplete();

                SoundManager.instance.StopMusic();
                // 3.a: We are all done processing the mesh, so we can now
                // initialize a collection of Placeable holograms in the world
                // and use horizontal/vertical planes to set their starting positions.
                // Call GameCollectionManager.Instance.GenerateItemsInWorld().
                // Pass in the lists of horizontal and vertical planes that we found earlier.
                DecorationsSpawnerManager.Instance.GenerateInitialItemsInWorld(floors, walls);
