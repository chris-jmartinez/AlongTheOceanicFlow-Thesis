...
namespace HoloToolkit.Examples.InteractiveElements
{
    ...
    /// </summary>
    public class SliderGestureControlCustom : GestureInteractiveControl
    {
        ...
        {
            //COMMENTO IN ITALIANO IN MODO DA DISTINGUERE MEGLIO I MIEI COMMENTI E LA MIA PARTE DI CODICE DA QUELLA DEL TOOLKIT
            //VERSIONE ORIGINALE AVEVA TUTTA QUESTA PARTE ATTIVA:
            base.Awake(); //Ho mantenuto solo questa call ad awake della classe padre
        ...
        public void SetSliderValueAndReinitAwake_Custom(float sliderValueToSet)
        {
            /*
             Modifico il codice del toolkit in quanto, usando la funzione pubblica SetSliderValue, al momento di caricare le sliderBars
             nella schermata infinity i knobs/manopole non venivano posizionati correttamente e non rispecchiavano il reale valore assunto
             dai parametri.
             Ho quindi deciso di reinizializzare le barre ogni volta che si carica la schermata infinity, eseguendo il codice presente in
             Awake() per ogni barra ogni volta che la schermata infinity viene visualizzata (oppure quando viene premuto il pulsante reset).
             
             Era presente un bug grafico: i knobs e valori funzionavano perfettamente ma il colore di ogni barra risultava non corretto.
             Ho risolto il bug grafico reinizializzando anche il valore x della proprietà localScale di SliderFill, il componente che si 
             occupa di colorare la barra di azzurro.
             */
            
           
            //CODICE MIO
            //Codice aggiunto per settare il valore desiderato
            sliderValue = sliderValueToSet;
            //Codice aggiunto per reinizializzare lo sliderFill (componente che colora di azzurro la barra)
            SliderFill.transform.localScale = new Vector3(1f,1f,1f);

           
            
            //Il contenuto di AWAKE() è stato riposizionato qui sotto, in modo da reinizializzare la barra ogni volta che viene caricata.

            //base.Awake();  //Commento questo base.Awake() in quanto voglio venga eseguito una sola volta nell'originaria funzione awake.
        ...
        {
            //Questa funzione SetSliderValue non funziona al fine di settare dinamicamente il valore dello slider (almeno, non funziona per i miei scopi)
            if (GestureStarted)
            {
                return;
            }
